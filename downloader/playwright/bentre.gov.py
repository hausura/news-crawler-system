...
async def process_page(page):
    ...
    for link in related_links:
        ...
        if href and 'https://bentre.gov.vn/Lis' in href and text.strip():
            if not collection.find_one({"url": href}):
                collection.insert_one({"url": href})
                found_url.add(href)
...
async def crawl_paginated_pages(playwright, start_url, max_pages=1473):
    ...
    try:
        ...
        await page.goto(start_url, timeout=65000)
...
async def main():
    start_url = "https://bentre.gov.vn/Lists/Tintucsukien/TongQuat.aspx"
